...
/// A degraded-only spec (an inline object field, nothing dropped) still loses
/// information, so `--strict` fails on it too.
#[test]
fn strict_fails_on_degraded() {
    let spec = r#"
openapi: "3.0.3"
info:
  title: Test
    ...
    Thing:
      ...
      properties:
        name:
          ...
          minLength: 1
"#;
    let status = run_strict(spec, "openapi_modelgen_strict_clean");
    assert!(status.success(), "clean spec should exit 0, got {status:?}");
}
